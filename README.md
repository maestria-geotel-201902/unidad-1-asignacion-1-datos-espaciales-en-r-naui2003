...
## Reading layer `ProvCenso2010' from data source `/home/naui2/unidad-1-asignacion-1-datos-espaciales-en-r-naui2003/data/divisionRD.gpkg' using driver `GPKG'
...
``` r
plot(prov[prov$PROV %in% '29', 'PROV'])
```
...
## 29   29  09 MONTE PLATA   0929 MULTIPOLYGON (((422830.4 21...
plot(miprov)
```
...
##           areaenm2
## 1 2602392844 [m^2]
`prov` %>% dplyr::filter(PROV %in% '29') %>% dplyr::select(`areaenm2`) %>% plot
```
...
``` r
`prov` %>% dplyr::filter(PROV %in% c('15', '25', '29')) %>% dplyr::select(`areaenm2`) %>% plot
```
...
## Reading layer `MunCenso2010' from data source `/home/naui2/unidad-1-asignacion-1-datos-espaciales-en-r-naui2003/data/divisionRD.gpkg' using driver `GPKG'
...
``` r
mimun <- mun %>% dplyr::filter(PROV %in% '29')
mimun %>% plot
```
